    ...
    void dequeue()//出队 
    {
        if (this->head != nullptr)
        {
            ...
            if (this->head == nullptr)
            {
             ...
             //返回第一个课程信息 
//    T front()   //常量成员函数不会修改队列对象的状态,即不会改变队列中的元素。
//    {                 
//        if (this->head != nullptr)
//        {
...
////		static int count;
//	public:
//		course(string _getclass):_class(_getclass){
//		}
//		static string getclas()
//		{
...
////			return;
////		}
//		void show(bool show_description = false)
//	    {
//	        if (show_description) {
...
//    ScheduleItem(const string& desc = "") : description(desc) {}
//
//    // 修改为接受 fstream 引用，适用于读写操作
//    void fwrite(fstream& stream) {
//        if (stream) {
...
//    
//
//    void fread(fstream& stream) {
//        getline(stream, description);
//    }
//
//
//	// 将当前对象的描述写入到文件流中
//void fwrite(ofstream& outStream) {
//if (outStream) {
...
//
//// 从文件流中读取描述信息
//void fread(ifstream& inStream) {
//    getline(inStream, description);
//}
//    static string getclas() {
//        return "ScheduleItem";  // 提供类名
//    }
//
//    void show(bool show_description = false) {
//        if (show_description) {
	...
	do
	{
		...
		if (choice==1)
		{
			...
			do{
				...
				if(choice_tch>=1&&choice_tch<=8)cout<<"操作完毕!\n(任意键继续)"<<endl;
				_getch();
		...
		else if(choice==2)
		{
			...
			do{
				...
				if(choice_stu>=1&&choice_stu<=8)cout<<"操作完毕!\n(任意键继续)"<<endl;
				_getch();
			}while(choice_stu);
		}
		else if(choice==3) 
    ...
    do 
	{
        ...
        switch (choicee) 
		{
            case 1:
                ...
                string description;
                cin>>description;
				schedule.addScheduleItem(description);
                break;
				}
            case 2:
...
void menu_schedule()
{
	cout<<endl<<"▼▼▼▼▼▼欢迎使用日程表▼▼▼▼▼▼"<<endl;
	cout<<"按0 ：退出"<<endl; 
	cout<<"按1 ：添加日程"<<endl;
	cout<<"按2 ：打卡日程"<<endl;
	cout<<"按3 ：查看今日所有日程"<<endl;
}
