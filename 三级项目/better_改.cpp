...
public:
	...
	//		}
			...
			{
				T t("");
				if(T::getclas()!=string("ScheduleItem"))
				{
					t.fread(pf);
...
void schedule(Schedule* _schedule)
{
		...
		if (choice==1)
		{
			...
			do{
				...
				switch (choice_tch)
				{
					...
					case 9:
						schedule(pschedule_tch);
						break;
					default:
						cout<<"输入有误，请重试"<<endl;
						break;
				}
				if(choice_tch>=1&&choice_tch<=9)cout<<"操作成功!\n(任意键继续)"<<endl;
				_getch();
